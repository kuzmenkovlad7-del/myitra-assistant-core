...
import { X, Mic, MicOff, Camera, CameraOff, Phone, Volume2, VolumeX, Sparkles, Brain } from "lucide-react"
...
const AI_CHARACTERS: AICharacter[] = [
  {
    ...
    gender: "male",
    description: {
      en: "Senior psychologist, specialization: cognitive behavioral therapy, extensive experience with anxiety and stress.",
      ru: "Старший психолог, специализация: когнитивно-поведенческая терапия, большой опыт работы с тревогой и стрессом.",
      uk: "Старший психолог, спеціалізація: когнітивно-поведінкова терапія, великий досвід роботи з тривогою та стресом.",
    },
    avatar:
  ...
  },
  {
    ...
    gender: "female",
    description: {
      en: "Clinical psychologist, specialization: anxiety, depression, workplace stress. Gentle and structured approach.",
      ru: "Клинический психолог, специализация: тревога, депрессия, стресс на работе. Бережный и структурный подход.",
      uk: "Клінічний психолог, спеціалізація: тривога, депресія, стрес на роботі. Дбайливий і структурований підхід.",
    },
    avatar:
  ...
  },
  {
    ...
    gender: "female",
    description: {
      en: "Psychotherapist, specialization: emotional regulation, trauma, relationships. Suitable for deeper work.",
      ru: "Психотерапевт, специализация: эмоциональная регуляция, травма, отношения. Подходит для глубокой работы.",
      uk: "Психотерапевт, спеціалізація: емоційна регуляція, травма, стосунки. Підходить для глибокої роботи.",
    },
    avatar:
...
function normalizeLang(code?: string): "uk" | "ru" | "uk" | "en" {
...
export default function VideoCallDialog({ isOpen, onClose, openAiApiKey, onError }: VideoCallDialogProps) {
  ...
  const lang = normalizeLang(activeLanguage.code)

  const languageDisplayName =
    lang === "uk" ? "Ukrainian" : lang === "ru" ? "Russian" : "English"

  const currentLocale = getLocaleForLanguage(lang)
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      if (voices.length) {
        getRefinedVoiceForLanguage(lang, "female")
        getRefinedVoiceForLanguage(lang, "male")
      }
    }
    load()
  ...
  async function requestMicrophoneAccess(): Promise<boolean> {
    if (typeof navigator === "undefined") {
      setSpeechError(t("Microphone access is not available in this environment. Please open the assistant in a regular browser window."))
      return false
    }
    ...
    if (!hasMediaDevices) {
      setSpeechError(t("Microphone access is not supported in this browser. Please use the latest version of Chrome, Edge or Safari."))
      return false
    }
    ...
    try {
      ...
      if (name === "NotAllowedError" || name === "PermissionDeniedError") {
        setSpeechError(t("Microphone is blocked in the browser. Please allow access in the site permissions and reload the page."))
      } else if (name === "NotFoundError" || name === "DevicesNotFoundError") {
        setSpeechError(t("No microphone was found on this device. Please check your hardware."))
      } else {
        setSpeechError(t("Could not start microphone. Check permissions in the browser and system settings, then try again."))
      }
      return false
    }
  }
  ...
  function extractAnswer(data: any): string {
    ...
    if (Array.isArray(data) && data.length > 0) {
      const first = data[0] ?? {}
      return (first.output || first.response || first.text || first.message || first.content || first.result || JSON.stringify(first))?.toString().trim()
    }
    if (typeof data === "object") {
      return (data.output || data.response || data.text || data.message || data.content || data.result || JSON.stringify(data))?.toString().trim()
    }
    return ""
  }
  ...
  function browserSpeak(text: string, gender: "male" | "female", onDone: () => void) {
    ...
    const voice = getRefinedVoiceForLanguage(lang, gender)
    if (voice) utterance.voice = voice

    const speechParams = getNativeSpeechParameters(lang, gender)
  ...
  async function speakText(text: string): Promise<void> {
    ...
    if (hasEnhancedVideo && speakingVideoRef.current && selectedCharacter.speakingVideo) {
      try { speakingVideoRef.current.currentTime = 0; await speakingVideoRef.current.play() } catch {}
    }
    ...
    try {
      if (shouldUseGoogleTTS(lang)) {
        try {
          const audioDataUrl = await generateGoogleTTS(cleaned, currentLocale, gender)
          ...
          await new Promise<void>((resolve) => {
            ...
            // iOS/Safari: playsinline attribute is safer than TS property
            try { audio.setAttribute("playsinline", "true"); audio.setAttribute("webkit-playsinline", "true") } catch {}
  ...
  async function handleUserText(text: string) {
    ...
    try {
      ...
      if (error?.name === "AbortError") errorMessage = t("Connection timeout. Please try again.")
        ...
        errorMessage = t("The video assistant is temporarily unavailable. Please contact support.")
      else errorMessage = t("I couldn't process your message. Could you try again?")
  ...
  function startSpeechRecognition() {
    ...
    if (!SpeechRecognition) {
      setSpeechError(t("Your browser does not support voice recognition. Please use Chrome or another modern browser."))
      return
    }
    ...
    recognition.onerror = (event: any) => {
      if (event.error === "not-allowed") {
        setSpeechError(t("Microphone access was blocked. Please allow it in your browser settings and restart the call."))
        setIsMicMuted(true)
    ...
    recognition.onend = () => {
      ...
      if (recognitionStopReasonRef.current === "none" && isCallActiveRef.current && !isMicMutedRef.current) {
        const now = Date.now()
  ...
  async function startCall() {
    ...
    try {
      ...
      if (!micOk) { setIsConnecting(false); return }
      ...
      setSpeechError(error?.message || t("Failed to start the call. Please check your microphone and camera permissions."))
  ...
  const statusText = (() => {
    ...
    if (micOn) return t("Listening...")
    return t("Paused. Turn on microphone to continue.")
  })()

  const getDescription = (c: AICharacter) => (lang === "ru" ? c.description.ru : lang === "uk" ? c.description.uk : c.description.en)

  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl h-[92dvh] sm:h-[min(90vh,860px)] overflow-hidden flex flex-col">
        {/* HEADER (fixed) */}
        <div className="shrink-0 p-3 sm:p-4 border-b flex justify-between items-center relative bg-gradient-to-r from-indigo-600 via-violet-600 to-sky-500 text-white">
        ...
        {/* CONTENT AREA (no jump, internal scroll) */}
          ...
          {!isCallActive ? (
            <div className="h-full flex flex-col">
              ...
              <div className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 touch-pan-y overscroll-contain">
                ...
                <div className="mb-5 bg-blue-50 p-4 rounded-lg w-full max-w-xs text-center mx-auto">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {AI_CHARACTERS.map((character) => (
                      <button
                        ...
                        className={`relative bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border-2 min-w-0 ${
                          selectedCharacter.id === character.id ? "border-indigo-600" : "border-transparent"
                        }`}
                      >
                        <div className="p-4 sm:p-5 flex flex-col h-full min-w-0">
                          ...
                          <h4 className="font-semibold text-base sm:text-lg text-center mb-1 sm:mb-2 truncate">
                            {character.name}
                          </h4>

                          <p className="text-xs sm:text-sm text-gray-600 text-center mb-3 sm:mb-4 break-words">
              ...
              {/* FIXED FOOTER BUTTON */}
              <div className="shrink-0 border-t bg-white/95 backdrop-blur px-3 py-3 sm:px-4 sm:py-4 pb-[calc(env(safe-area-inset-bottom)+12px)]">
                <div className="w-full max-w-md mx-auto px-2">
                  <Button
                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-base sm:text-lg py-5 min-h-[56px]"
                    onClick={startCall}
                  ...
                  {speechError && (
                    <p className="mt-3 text-xs text-center text-rose-600 break-words">{speechError}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="h-full p-3 sm:p-4">
              <div className="h-full flex flex-col sm:flex-row gap-3 sm:gap-4 min-h-0">
                {/* LEFT: VIDEO */}
                <div className="w-full sm:w-2/3 flex flex-col min-h-0 min-w-0">
                  <div className="relative w-full aspect-video sm:flex-1 min-h-0 bg-white rounded-lg overflow-hidden">
                ...
                {/* RIGHT: CHAT */}
                <div className="w-full sm:w-1/3 flex flex-col min-h-0 min-w-0 bg-gray-50 rounded-lg border overflow-hidden">
                  <div className="px-3 pt-3 pb-2 border-b flex items-center gap-2 min-w-0">
                  ...
                  <div className="flex-1 min-h-0 px-3 py-3 sm:px-4 sm:py-4 space-y-3 sm:space-y-4 overflow-y-auto">
                    {messages.length === 0 && (
                      <div className="bg-indigo-50 rounded-2xl p-3 sm:p-4 text-xs sm:text-sm text-slate-800 break-words">
                    ...
                    {messages.map((msg) =>
                      msg.role === "user" ? (
                        <div
                          key={msg.id}
                          className="ml-auto max-w-[85%] rounded-2xl bg-blue-50 px-3 py-3 text-xs sm:text-sm text-slate-900 break-words"
                        >
                        ...
                        <div
                          key={msg.id}
                          className="max-w-[85%] rounded-2xl bg-emerald-50 px-3 py-3 text-xs sm:text-sm text-slate-900 break-words"
                        >
        ...
        {/* BOTTOM BAR (fixed in-call) */}
        {isCallActive && (
          <div className="shrink-0 border-t bg-gray-50 px-3 py-3 sm:px-4 sm:py-4 pb-[calc(env(safe-area-inset-bottom)+12px)]">
            <div className="flex items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-2 text-[11px] sm:text-xs text-slate-500 min-w-0">
