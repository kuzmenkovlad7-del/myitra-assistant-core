...
export default function VoiceCallDialog({
  ...
  const sessionLangRef = useRef<string>(computeLangCode())
  ...
  async function sendChunkToStt(blob: Blob) {
    ...
    try {
      ...
      if (data?.lang) {
        const detected = sttLangToLocale(data.lang)
  ...
  async function handleUserText(text: string) {
    const voiceLangCode = sessionLangRef.current || computeLangCode()
    const langCode = localeToShort(voiceLangCode)
        ...
        <div className="overflow-hidden rounded-3xl bg-white shadow-xl shadow-slate-900/10">
          ...
          <div className="flex h-[500px] flex-col md:h-[540px]">
            <ScrollArea className="flex-1 px-5 pt-4 pb-2">
              <div
                ref={scrollRef}
                className="max-h-full space-y-3 pr-1 text-xs md:text-sm notranslate"
