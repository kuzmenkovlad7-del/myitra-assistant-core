...
export default function VoiceCallDialog({
  ...
  async function maybeSendStt() {
    ...
    try {
      ...
      dlog("[STT] sending blob size=", blob.size, "type=", blob.type)
  ...
  const startCall = async (gender: "female" | "male") => {
    ...
    try {
      ...
      if (typeof MediaRecorder === "undefined") {
        setNetworkError(
          t(
            "Audio recording is not supported in this browser. Please use Chrome/Edge/Safari (latest).",
          ),
        )
        setIsConnecting(false)
        return
      ...
      if (track) {
        dlog("[MIC] label:", track.label, "readyState:", track.readyState)
        track.onended = () => {
      ...
      recorder.onstart = () => {
        setIsListening(true)
        dlog("[REC] start", recorder.mimeType || options.mimeType || "unknown")
      }
      ...
      recorder.ondataavailable = (event: BlobEvent) => {
        ...
        dlog("[REC] chunk size=", b.size, "totalChunks=", audioChunksRef.current.length)
        void maybeSendStt()
      }
      ...
      recorder.onerror = (event: any) => {
        dlog("[REC] error", event?.name || "", event?.message || "")
      }
      ...
      // 3000мс — стабильнее, чем 250/350мс (на ПК/Сафари меньше шанс нулевых чанков)
      recorder.start(3000)
    } catch (error: any) {
